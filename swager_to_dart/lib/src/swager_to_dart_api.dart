...
Future<List<ApiModel>> _getApis(Map<String, dynamic> map) {
  ...
  map.forEach(
    (key, value) {
      ...
      var s = m["post"]["summary"];
      var o = m["post"]["operationId"];
      ...
      apis.add(
        ApiModel(
          ...
          tag: t,
          //  Comments here
          summary: s+"\n\/\/\/\n"+p.map((e) => "\/\/\/  Where [${e.name}] is ${e.description}").join("\n\/\/\/\n"),
          operationId: o,
          params: p,
...
String _toClassName(String name) {
  var f = name.characters.first.toUpperCase();
  ...
  var api = apis.map<String>(
    (e) {
      ...
      d = d.replaceAll(
        ...
        e.params.length != 0
            ? "{${e.params
                .select((x, i) => "@required ${x.type} ${x.name}")
                .join(",")}}"
            : "",
      );
      d = d.replaceAll(
